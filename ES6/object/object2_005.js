...
// [1] : 자바스크립트는 객체지향 언어인가?
// 그렇다 --> 다만 다른언어와는 달리 프로토타입 상속에 기반을 둔 OOP(object-oriented programming - 객체지향언어) 언어. --> Class 문법이 지원 --> 다른 언어와 비슷.
...
// [3] : 프로토타입 체인
// __proto__ --> 상속을 해준 부모(원형)를 가르킴 (참조).
// 이 말은 자식 객체가 [Prototype]가 가르키는 부모 객체의 멤버(속성, 메서드)를 사용할 수 있다. --> 즉, 상속 받았다.
...
// 여기서 문제? --> obj2 객체의 [Prototype] 속성이 가리키는 것은? 
console.log(obj2); // Object(최상위)
console.log(obj2.name); // 이순신

// 여기서 문제? --> 그럼 [Prototype](__proto__) 속성이 가리키는 것을 바꿀 수 있는가? --> YES
obj2.__proto__ = obj1;
console.log(obj2); 			// obj1 (부모) --> obj1이 가지는 멤버를 사용할 수 있게 됨.
console.log(obj2.name); 	// 이순신 --> 애가 출력된 이유는 obj2 객체 내에 name 속성이 있으니깐 부모까지 찾으러 안가고 출력
console.log(obj2.age);		// 20 --> 애가 출력된 이유는 obj2 객체 내에 age 속성이 없으니깐 부모까지 찾아 올라감.
obj2.sayHi();				// Hi~ 이순신

// [4] : 빈 객체 생성 후 --> __proto__ 속성에 원하는 객체(부모 or 원형이 될)를 할당 --> 상속
console.clear();
let obj3 = {};
...
// 4. 이와같이, JS는 프로토타입 체인을 사용하여 객체의 특성(속성, 메서드를)을 다른 객체로 전파 --> "프로토타입 상송"
